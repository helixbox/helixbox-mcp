...
server.tool("bridge", "Bridge tokens", {
    fromChain: z.number().describe("Source chain ID"),
    toChain: z.number().describe("Target chain ID"),
    fromToken: z.string().describe("Source token address"),
    toToken: z.string().describe("Target token address"),
...
server.tool("chains", "Get supported chains", {}, async () => {
    try {
...
server.tool("tokens", "Get supported tokens", {
    chains: z.array(z.union([z.string(), z.number()])).optional().describe("List of chain IDs or keys (optional)"),
}, async ({ chains }) => {
    try {
        const tokens = await getTokens(chains ? { chains: chains } : undefined);
        return {
            content: [
                {
                    type: "text",
...
server.tool("tools", "Get supported bridges and exchanges", {
    chains: z.array(z.union([z.string(), z.number()])).optional().describe("List of chain keys or IDs (optional)"),
}, async ({ chains }) => {
    try {
        const tools = await getTools(chains ? { chains: chains } : undefined);
        return {
            content: [
                {
                    type: "text",
...
server.tool("quoteToAmount", "Get a quote for a token transfer using toAmount", {
    fromChain: z.number().describe("Source chain ID"),
    toChain: z.number().describe("Target chain ID"),
    fromToken: z.string().describe("Source token address"),
    toToken: z.string().describe("Target token address"),
...
server.tool("connections", "Get all available connections for swapping or bridging tokens", {
...
server.tool("tokenBalance", "Get the balance of a specific token for a wallet", {
...
server.tool("gasPrice", "Get gas price for a specific chain", {
    chainId: z.number().describe("Chain ID"),
}, async ({ chainId }) => {
    try {
